...
class AIPlayer(Player):
    ...
    def __init__(self, epsilon=0.4, alpha=0.3, gamma=0.9, default_q=1):
        ...
        # the Q(s, a) function in a neural network (this is how we eliminate the huge data structures)
    ...
    def encode_input(self, board, action):
        # we represent the state and action pair with one dimensional array
        vector_representation = []

        # one hot encoding for three states
    ...
    # lets evaluate a given state: so update the Q(s, a) table regarding its state and a action
    def reward(self, reward, board):
        if self.move:
            ...
            # train a neural network with the new (s, a) and Q values
            self.q.fit(self.encode_input(self.board, self.move), prev_q + self.ALPHA * ((reward + self.GAMMA * max_q_new)-prev_q), epochs=3, verbose=0)
